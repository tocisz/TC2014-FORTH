...
\ args under dptr: 0) 1st next state 2) 1st letter 3)
\ 3) 2nd next state 4) 2nd letter
: rFork ( key dptr - state flag )
	>r ( key R:dptr )
	dup r@ 2+ c@ = if ( key )
		\ recognized 1st
		drop r> ( dptr )
...
: handleAction ( pos flag - pos flag )
	...
	and if ( pos flag )
		...
		exit
	then ( pos flag )
	dup a:do = if ( pos flag )
		1 mulOn !
		exit
	then
	dup a:don't = if ( pos flag )
		0 mulOn !
		exit
	then
;
